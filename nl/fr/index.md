...
Pour plus d'informations sur la tarification, voir [{{site.data.keyword.backup_notm}} storage ](https://www.ibm.com/cloud/backup-and-restore){:new_window} et [{{site.data.keyword.backup_notm}} on IBM Cloud](https://www.ibm.com/cloud/evault/pricing){:new_window}.
...
1. Dans la [console {{site.data.keyword.cloud_notm}}](https://{DomainName}/catalog/){:new_window}, cliquez sur l'icône **Menu** dans l'angle supérieur gauche. Sélectionnez **Infrastructure classique**.
...
2. Examinez la section {{site.data.keyword.backup_notm}} et vérifiez que la taille sélectionnée lors de la procédure d'acquisition est affichée.
   ...
   Si la taille du stockage de sauvegarde continue à afficher une capacité égale à zéro, actualisez à nouveau la page.{:tip}

## Accès et examen des détails du stockage du service de stockage {{site.data.keyword.backup_notm}}

Vous pouvez examiner les détails du stockage de votre service {{site.data.keyword.backup_notm}} dans la [console {{site.data.keyword.cloud_notm}} ](https://{DomainName}/catalog/){:new_window} et le portail {{site.data.keyword.slportal}} ou à tout moment. Les détails que vous pouvez consulter comprennent le mot de passe, l'adresse de stockage et l'utilisation qui sont associés au service {{site.data.keyword.backup_notm}} sélectionné.
...
- [Installation du client de sauvegarde dans Linux](install-backup-client-linux.html)
- [Installation du client de sauvegarde dans Windows](install-backup-client-windows.html)
- [Installation du client de sauvegarde pour Windows 2016](install-windows2016.html)

## Accès à WebCC (WebCentralControl)

WebCC (WebCentralControl) est le client qui est utilisé pour interagir avec un service {{site.data.keyword.backup_notm}} offert par {{site.data.keyword.BluSoftlayer_full}}. WebCC est un client reposant sur un navigateur qui s'exécute sur le réseau privé {{site.data.keyword.BluSoftlayer_full}} et permet un contrôle complet d'un service {{site.data.keyword.backup_notm}}, notamment pour la configuration et les restaurations. Procédez comme suit pour accéder à WebCC :

1. Accédez au réseau privé via VPN.

   WebCC n'est pas accessible à travers le réseau public. Une connexion VPN doit d'abord être établie.
   {:important}
2. Accédez à l'écran de stockage de sauvegarde sur le portail [{{site.data.keyword.slportal}} ![External link icon](../../icons/launch-glyph.svg "External link icon")](https://control.softlayer.com/){:new_window}.
...
Pour plus d'informations sur les schémas de conservation, voir la [Foire aux questions](faqs.html).   {:tip}
...
Pour plus d'informations sur les sauvegardes de niveau fichier sous Linux, voir [Configuration d'une sauvegarde simple au niveau fichier sur Linux](configure-simple-file-backup-linux.html).
{:tip}
