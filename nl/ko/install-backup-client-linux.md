...
Linux 기반 운영 체제에서 {{site.data.keyword.backup_full}} 클라이언트 설치는 OS 내의 쉘이나 터미널에서 일련의 명령을 통해 수행될 수 있습니다. 이 프로시저에서는 다음의 Linux 기반 운영 체제에서 {{site.data.keyword.backup_notm}} 클라이언트를 설치하는 데 필요한 단계를 간략하게 설명합니다. 
...
[{{site.data.keyword.cloud_notm}} 카탈로그](https://console.bluemix.net/catalog/){:new_window} 또는 {{site.data.keyword.slportal}}을 통해 서버를 주문할 때 {{site.data.keyword.backup_notm}}을 구매한 경우에는 소프트웨어가 자동으로 설치됩니다. 이 문서에 설명된 프로시저를 사용하지 않아도 됩니다.
{:tip}
...
## 백업 클라이언트 설치 준비를 위한 Linux 업데이트(RedHat Linux에만 해당됨)
이 단계는 RedHat Linux의 경우에는 필수이지만 기타 Linux 배포의 경우에는 선택사항입니다.
{:important}
