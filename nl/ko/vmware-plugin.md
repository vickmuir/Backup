...
3. 볼트(vault) 정보를 구성하십시오.

   볼트 정보는 **자동** 또는 **수동**으로 구성할 수 있습니다.<br/>에이전트를 처음에 구성하는 경우 [자동 구성](#VRAautoconfig) 옵션을 사용하여 구성 가능합니다.<br/>컴퓨터가 이전에 볼트에 등록된 경우 자동 구성이 작동하지 않으며 볼트를 [수동으로 구성](#VRAmanualconfig)해야 합니다.
   {: tip}
...
볼트 설정을 자동으로 구성하려면 **자동으로 구성**을 클릭하십시오. 구성 마법사가 실행되고 사용자를 대신하여 볼트를 구성합니다. 완료되면 **에이전트로 이동**을 클릭하여 **볼트 설정** 탭의 정보를 확인하십시오.

### 볼트 수동 구성
{: #VRAmanualconfig}

볼트 설정을 수동으로 구성하려면 **수동으로 구성**을 클릭하십시오.   
1. **볼트 설정**을 클릭하십시오.
2. **볼트 추가**를 클릭하십시오. 볼트 설정 창이 나타납니다. **볼트 프로파일** 옵션에서 적절한 볼트를 선택하십시오.
   컴퓨터가 이전에 {{site.data.keyword.backup_notm}} 볼트에 등록된 경우, 볼트 프로파일에서 볼트가 선택되면 모든 정보가 자동으로 채워집니다.
   {:note}

3. 볼트 설정 페이지의 모든 정보를 확인하고 **저장**을 클릭하십시오.
4. 볼트 설정을 저장하면 **볼트 설정** 탭에서 볼트 정보를 볼 수 있습니다.


### vCenter 에이전트 설정 구성
{: #vCenterSettingsconfig}
볼트 등록이 완료된 후에 vCenter 설정을 구성해야만 백업 작업을 작성하고 실행할 수 있습니다.
