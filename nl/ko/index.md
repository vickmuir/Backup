...
백업은 데이터가 디바이스 외부에 안전하게 저장되어 데이터 유실 시에 보호되도록 보장합니다. EVault 백업은 EVault WebCC 브라우저 기반 관리 유틸리티를 통해 관리되는 자동화된 에이전트 기반 백업 시스템입니다. EVault는 {{site.data.keyword.BluSoftlayer_full}} 네트워크의 하나 이상의 데이터 센터에서 서버 간에 데이터를 백업하는 방법을 사용자에게 제공합니다. 관리자는 전체 시스템, 특정 디렉토리 또는 심지어 개별 파일을 대상으로 하는 일별, 주별 또는 사용자 정의 스케줄에 따라 백업을 설정할 수 있습니다. 추가 플러그인은 Microsoft Exchange 및 Microsoft SQL 등의 소프트웨어, 기타 유형의 서드파티 소프트웨어와의 호환성을 보장하며, 사용자는 이를 사용하여 필요 시에 Bare Metal Restore를 완료할 수 있습니다.
...
가격에 대한 정보는 [백업 스토리지](https://www.ibm.com/cloud/backup-and-restore){:new_window} 및 [EVault on IBM Cloud](https://www.ibm.com/cloud/evault/pricing){:new_window}를 참조하십시오.
...
8. 오른쪽에서 **클라우드 서비스 이용 약관** 및 **서드파티 서비스 계약** 선택란을 클릭하십시오.
...
1. **EVault 백업 스토리지** 화면에 액세스하려면 고유 인증 정보를 사용하여 [{{site.data.keyword.slportal}}](https://control.softlayer.com/){:new_window}에 로그인하십시오.
